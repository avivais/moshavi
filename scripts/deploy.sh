...
ssh -i "$SSH_KEY" "$SSH_USER@$SSH_HOST" "cd $APP_PATH && git stash push -m 'pre-deploy' -- package.json package-lock.json 2>/dev/null; git pull && npm ci && npm run build && mkdir -p public/media/gallery public/media/gallery/thumbs && (npm run db:setup 2>/dev/null || true) && (npm run fix-posters 2>/dev/null || true) && pm2 restart moshavi"
